...
def get_meta_data(target_id, username: str):
...
def fetch_reels_paginated(target_id, username: str, desired_count: int = 20, sleep_seconds: float = 1.0, max_per_page: int = 50):
    """Fetch up to `desired_count` reels by paginating the GraphQL endpoint.

    This function will perform repeated requests, respect a small sleep between
    requests to reduce chance of rate limiting, and write a combined
    `meta_data.json` at `output_json/<username>/meta_data.json`.
    ...
    # Base payload fields copied from the single-call implementation
    base_payload = {
        "av": "17841477778895962",
        "__d": "www",
        "__user": "0",
    ...
    headers = {
        ...
        "accept-encoding": "gzip, deflate, br, zstd",
        "accept-language": "en-US,en;q=0.9",
        # include cookie copied from the single-call implementation; session must be valid
...
def main():
    ...
    usernames = {"ajaydevgn","kartikaaryan"}
    for username in usernames:
        ...
        #meta_output_path = get_meta_data(target_id, username)
        meta_output_path = fetch_reels_paginated(target_id, username, desired_count=100, sleep_seconds=3.0, max_per_page=50)
        if meta_output_path:
        ...
        #print("Sleeping for 29 seconds to avoid rate limiting...")
        #time.sleep(29)
